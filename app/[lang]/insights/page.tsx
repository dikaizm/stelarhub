...
export default async function InsightsPage({ params }: { params: Promise<{ lang: Language }> }) {
    ...
    return (
        <main className="pt-16 min-h-screen bg-background">
            <Section className="!pb-0 text-center">
                <div className="max-w-4xl mx-auto">
                    <h1 className="text-4xl md:text-6xl font-bold text-text mb-6">
                        {dict.insightsPage.hero.title}
                    </h1>
                    <p className="text-xl text-text-secondary max-w-2xl mx-auto">
                        {dict.insightsPage.hero.subtitle}
                    </p>
                </div>
            </Section>
            ...
            <Section background="white">
                ...
                {featuredPost && (
                    <div className="mb-16">
                        <Link href={`/${lang}/insights/${featuredPost.slug}`} className="block group">
                            <div className="grid md:grid-cols-2 gap-8 items-center overflow-hidden rounded-2xl bg-slate-50 p-4 hover:bg-slate-100 transition-all">
                                ...
                                <div className="px-6 py-8 md:px-12 md:py-20">
                                    ...
                                    <span className="text-primary font-semibold flex items-center gap-2 group-hover:gap-3 transition-all">
                                        {dict.insightsPage.readFull} <ArrowRight size={18} />
                                    </span>
                                </div>
                            </div>
                        </Link>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                    {recentPosts.length > 0 ? (
                        ...
                        <>
                            {!featuredPost && <div className="col-span-full text-center text-text-secondary">{dict.insightsPage.noInsights}</div>}
                        </>
                    )}
                </div>
            </Section>
